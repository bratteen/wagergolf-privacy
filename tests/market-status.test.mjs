...
test('explicit marknad använder samma Sverige-only-grind som nedladdningen', async () => {
  assert.deepStrictEqual((await state('?m=SE', 'DK')).body, { market: 'SE', public: true });
  assert.deepStrictEqual((await state('?m=DK', 'SE')).body, { market: 'DK', public: false });
});

test('GeoIP öppnar Sverige och håller övriga målmarknader stängda', async () => {
  assert.deepStrictEqual((await state('', 'SE')).body, { market: 'SE', public: true });
  for (const country of ['DK', 'NO', 'IE', 'FI', 'NL', 'AT', 'PT', 'BE', 'DE', 'FR', 'ES', 'IT']) {
    assert.deepStrictEqual((await state('', country)).body, { market: country, public: false }, country);
  }
});

test('okänt land är fail-closed och svaret får inte delas mellan länder', async () => {
  const { response, body } = await state('?m=US', 'SE');
  assert.deepStrictEqual(body, { market: null, public: false });
...
test('språk utan verifierad marknad öppnar aldrig en storefront', async () => {
  assert.deepStrictEqual((await state('?l=sv')).body, { market: null, public: false });
  assert.deepStrictEqual((await state('?l=en')).body, { market: null, public: false });
});
...
test('market-status använder Workers request.cf.country utan GeoIP-header', async () => {
  ...
  assert.deepStrictEqual(await response.json(), { market: 'SE', public: true });
});
