---
slug: foursome
key: guide:foursome
format: Foursome
category: Team
players: "4 players, 2 teams"
order: 2
title: "Foursome in golf: rules, tactics and handicap | Wager Golf"
description: "Foursome, or alternate shot, explained. One ball per team, partners play every other shot. Rules, who tees off on which holes, tactics and WHS team handicap."
h1: "Foursome in golf"
lede: "One ball per team, and you play alternate shots. One of you tees off on the odd holes, the other on the evens. No format binds a partnership tighter."
cardText: "One ball per team, partners play alternate shots. Odd and even holes."
faq:
  - q: "Who tees off on which holes?"
    a: "The team decides in advance who tees off on the odd holes and who takes the evens. That order holds for the whole round, regardless of who holed the putt on the previous hole."
  - q: "What happens after the tee shot?"
    a: "Your partner plays the next shot, and then you alternate shot by shot until the ball is holed. Putts count as ordinary strokes, so those alternate too."
  - q: "How is team handicap worked out?"
    a: "In Foursome stroke play, 50 per cent of the sum of the partners' course handicaps is used. That gives the team a combined allowance, deducted from the team's gross score."
  - q: "Why is it called a foursome?"
    a: "The name refers to the four players in a match: two teams of two. It should not be confused with Four-Ball, where all four play their own ball."
  - q: "Is a foursome difficult?"
    a: "It is the most demanding team format, because you always play the position your partner left you. A missed tee shot punishes your partner immediately. That is exactly why it is also one of the most bonding games to play, and why it features in the Ryder Cup."
related: [greensome, scramble, bastboll]
---

## What is a Foursome?

Foursome, also called alternate shot, is the classic and most demanding team format. The team shares a single ball and plays alternate shots. That means you have to trust your partner, because you always play the position they leave behind. Few formats create the same togetherness, or the same nerves over a short putt. There is a reason Foursome is one of the formats in the Ryder Cup.

## How to play a Foursome

- Decide who tees off on the odd holes and who takes the evens.
- After the tee shot your partner plays the next stroke.
- You alternate shot by shot, putts included, until the ball is holed.
- The team has one score per hole, that of the shared ball.

## How it is worked out

Two teams meet, either as a match hole by hole or on the total. Because each team only has one ball, every shot matters — a missed tee shot punishes your partner immediately.

The team handicap in Foursome stroke play is 50 per cent of the sum of the partners' course handicaps.

## Tactics: who tees off where

Choosing who tees off on the odd and even holes is the most important decision in a Foursome. Look at the course in advance: do you want your longest player on the par 5 tees and the straightest on the tight par 4s? Think about the par 3s too, where whoever tees off is also hitting the green. Give those to whoever is steadiest with the irons. A well-considered split can be worth several shots over a round.

## Foursome, Greensome or Four-Ball?

Three formats often get confused. In a Foursome only one of you tees off and you share a ball. In a Greensome both tee off, you pick the better drive and then share a ball. In Four-Ball everyone plays their own ball the whole way. Foursome is therefore the purest shared game, and the most demanding.

Wager Golf keeps track of whose turn it is to tee off, works out the team allowance and decides the match for you.

## Common mistakes

- **Not planning who tees off where.** Split the odd and even holes to suit you — who is steadiest on the par 3s, who is longest on the par 5s. It can be worth several shots.
- **Apologising for every miss.** In a Foursome you always play your partner's position. Dwell on the mistakes and you will both lose focus. Play on, the next shot is yours.

<figure class="guide-figure">
  <img src="{% shot "live" %}" alt="Wager Golf scorecard during a foursome match" width="640" height="1306" loading="lazy">
  <figcaption>The app keeps track of whose turn it is to tee off and shows the match score live.</figcaption>
</figure>
